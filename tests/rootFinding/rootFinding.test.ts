...
describe('findAllRealRoots', () => {
  ...
  const testCases: [string, Polynomial, number[]][] = [
    ...
    // ['squarefree polynomial with multiple distinct non-integer roots', [-21.7647, -8.85633, 21.6047, 2.11051, -5.58579], [-1.4142, -1, 1.5, 2.7, 3.8]],
  ...
  it ('should find all the real roots of a squarefree polynomial with multiple distinct non-integer roots', () => {
    const polynomial = createPolynomial([-21.7647, -8.85633, 21.6047, 2.11051, -5.58579]);
    const expectedRoots = [-1.4142, -1, 1.5, 2.7, 3.8];
    const roots = findAllRealRoots(polynomial, 1e-8);
