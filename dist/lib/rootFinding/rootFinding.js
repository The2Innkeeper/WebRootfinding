 ...
 */
export function findAllRealRoots(polynomial, precision = 1e-5) {
    const roots = [];
    ...
    const processRoots = (polynomial, isNegative = false) => {
        ...
        const evaluateFunc = (x) => evaluatePolynomial(polynomial, x);
...
//   return roots;
// }
export function findStrictlyPositiveRoots(polynomial, precision) {
    ...
    const evaluateFunc = (x) => evaluatePolynomial(squareFreePolynomial, x);
...
export function findStrictlyNegativeRoots(polynomial, precision) {
    ...
    const evaluateFunc = (x) => evaluatePolynomial(negatedPolynomial, x);
    for (const interval of isolatedRootIntervals) {
        const root = refineRootIntervalBisection(evaluateFunc, interval, precision);
