...
export function lmqPositiveUpperBound(polynomial) {
    ...
    for (let neg_i = degree - 1; neg_i >= 0; neg_i--) {
        ...
        for (let pos_i = neg_i + 1; pos_i <= degree; pos_i++) {
            ...
            const radical = Math.pow(Math.pow(2, usageCounts[pos_i]) * Math.abs(coefficients[neg_i]) / coefficients[pos_i], 1.0 / (pos_i - neg_i));
...
export function lmqPositiveLowerBound(polynomial) {
    ...
    for (let neg_i = 1; neg_i <= degree; neg_i++) {
        ...
        for (let pos_i = neg_i - 1; pos_i >= 0; pos_i--) {
            ...
            const radical = Math.pow(Math.pow(2, usageCounts[pos_i]) * Math.abs(coefficients[neg_i]) / coefficients[pos_i], 1.0 / (neg_i - pos_i));
            minRadical = Math.min(minRadical, radical);
            usageCounts[pos_i]++;
        }
        if (minRadical !== Number.POSITIVE_INFINITY) {
